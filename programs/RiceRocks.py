...
def group_group_collide(group, other_group):
    counter = 0
    for sprite in set(group):
        collision = group_collide(other_group, sprite)
        if collision:
            group.remove(sprite)
            counter += 1
    return counter 
...
def draw(canvas):
    ...
    # increase rock's velocity as score increases
    for rock in rock_group:
        rock.vel[0] = rock.init_vel[0] + (rock.init_vel[0] * score * 0.02)
        rock.vel[1] = rock.init_vel[1] + (rock.init_vel[1] * score * 0.02)
...
# timer handler to spawn a rock (if < 12 rocks and game running), not too close
def rock_spawner():
